function [subsY,instrY,instructions]=sparsity_min(obj,thisExp)
%   Computes the sparsity pattern for an elementary expression
%   'thisExp' of type 'min_nz', allocates the required memory, and
...
for i=1:length(iSmall)
    ...
    if length(operands)==prod(osize1(dimension))
        if length(operands)>1
            instrY(i)=newInstruction(obj,obj.Itypes.I_min,[],operands,thisExp);
        else
            instrY(i)=operands;
        end
    else
        instrY(i)=newInstruction(obj,obj.Itypes.I_min0,[],operands,thisExp);
    end
end
