function out=ipmPD_CSsimple(pars)
...
if any(isSensitivity)
    error('ipmPD_CSsimple: isSensitivity=true not implemented, use ipmPD_CS.m \n');
end
if smallerNewtonMatrix==true
    error('ipmPD_CSsimple: smallerNewtonMatrix=true not implemented, use ipmPD_CS.m \n');
end
if skipAffine==false
    error('ipmPD_CSsimple: skipAffine=false not implemented, use ipmPD_CS.m \n');
end
if useLDL==false
    error('ipmPD_CSsimple: useLDL=false not implemented, use ipmPD_CS.m \n');
end
if atomicFactorization==true
    error('ipmPD_CSsimple: atomicFactorization=true not implemented, use ipmPD_CS.m \n');
end
if debugConvergence==true
    error('ipmPD_CSsimple: debugConvergence=true not implemented, use ipmPD_CS.m \n');
end
...
fprintf('\n  Starting ipmPD_CSsimple symbolic computations...\n');
t1=clock();
...
if nF>0
    ...
    newLambda=lambda+alphaDualIneq*dLambda;

    maxAlphaPrimal=clp(F,F_u*dU);
    maxAlphaDualIneq=clp(lambda,dLambda);
...
fprintf('(%.2f sec)\n    ',etime(clock(),t2));
fprintf('  done ipmPD_CSsimple symbolic computations (%.3f sec)\n',etime(clock(),t1));
