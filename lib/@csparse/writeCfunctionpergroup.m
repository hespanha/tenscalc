function writeCfunctionpergroup(obj,minInstructions4loop,maxInstructionsPerFunction,Cfunction,Hfunction,logFile,folder,profiling)
    ...
    for k=1:length(obj.copies(i).destination)
        ...
        if ~isequal(subscriptsDestination,subscriptsSource)
            if strcmp(obj.copies(i).functionName,'initDualEqX__')
                ...
                s1=setdiff(subscriptsDestination,subscriptsSource);
                if ~isempty(s1)
                    fprintf('\nwriteCfunctionpergroup: indices in subscriptsDestination and not in subscriptsSource:');
                    disp(s1)
                end
                s2=setdiff(subscriptsSource,subscriptsDestination);
                if ~isempty(s2)
                    fprintf('\nwriteCfunctionpergroup: indices in subscriptsSource and not in subscriptsDestination:');
                    disp(s2)
                end
                error('writeCfunctionpergroup: source and destination for Copy command ''%s'' with different sparsity structures, *most likely caused by equality constraints that do not involve optimization variables* or *equality constraints that trivially "resolve" optimization variables*',obj.copies(i).functionName);
            else
                subscriptsDestination,subscriptsSource,
