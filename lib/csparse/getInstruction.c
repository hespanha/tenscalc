...
/* mex -largeArrayDims COPTIMFLAGS="-Ofast /arch:AVX2 -DNDEBUG" CFLAGS="\$CFLAGS -Wall -Werror -Wno-unused-variable -Wno-unused-result -std=gnu99" "getInstruction.c" -outdir  */
                  ...
                  int nrhs, const mxArray *prhs[])
{
   ...
   if (mxIsSparse(prhs[0]))
       mexErrMsgIdAndTxt("getInstruction:prhs","input 1 (index) cannot be sparse (use full())");
   if (!mxIsInt64(prhs[0]))
   ...
   if (!PgetInstruction4MEX) {
     ...
     if (!PgetInstruction4MEX) { printf("[%s] Unable to get symbol: %s\n",__FILE__, dlerror());return; }// else { printf("[%s] Got symbol: getInstruction4MEX = 0x%" PRIXPTR"\n",__FILE__, PgetInstruction4MEX);}
#elif __APPLE__
