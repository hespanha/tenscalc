...
/* mex -largeArrayDims COPTIMFLAGS="-Ofast /arch:AVX2 -DNDEBUG" CFLAGS="\$CFLAGS -Wall -Werror -Wno-unused-variable -Wno-unused-result -std=gnu99" "appendInstruction.c" -outdir  */
                  ...
                  int nrhs, const mxArray *prhs[])
{
   ...
   if (mxIsSparse(prhs[0]))
       mexErrMsgIdAndTxt("appendInstruction:prhs","input 1 (type) cannot be sparse (use full())");
   if (!mxIsInt32(prhs[0]))
   ...
   if (mxIsSparse(prhs[1]))
       mexErrMsgIdAndTxt("appendInstruction:prhs","input 2 (parameters) cannot be sparse (use full())");
   ...
   if (mxIsSparse(prhs[2]))
       mexErrMsgIdAndTxt("appendInstruction:prhs","input 3 (operands) cannot be sparse (use full())");
   if (!mxIsInt64(prhs[2]))
   ...
   if (!PappendInstruction4MEX) {
     ...
     if (!PappendInstruction4MEX) { printf("[%s] Unable to get symbol: %s\n",__FILE__, dlerror());return; }// else { printf("[%s] Got symbol: appendInstruction4MEX = 0x%" PRIXPTR"\n",__FILE__, PappendInstruction4MEX);}
#elif __APPLE__
