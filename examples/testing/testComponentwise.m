...
% function, derivative
fun={@srelu,@(x)exp(x)./(1+exp(x)),@(x)exp(x)./(1+exp(x)).^2;
     @normpdf,@(x)-x.*exp(-x.^2/2)/sqrt(2*pi),@(x)(x.^2-1).*exp(-x.^2/2)/sqrt(2*pi);
     ...
     @atan,@(x)1./(1+x.^2),@(x)-2*x./(1+x.^2).^2;
     @(x)power(x,4),@(x)4*power(x,3),@(x)12*power(x,2);
     @round,@(x)zeros(size(x)),@(x)zeros(size(x));
     @ceil,@(x)zeros(size(x)),@(x)zeros(size(x));
     @floor,@(x)zeros(size(x)),@(x)zeros(size(x));
     @sign,@(x)zeros(size(x)),@(x)zeros(size(x));
     @abs,@sign,@(x)zeros(size(x));
     @relu,@heaviside,@(x)zeros(size(x));
     };
...
for f=1:size(fun,1)
    for i=1:length(gen)
        ...
        if any(abs(d2y-d2y_)>100*eps,'all')
            d2y,d2y_,
            error('error in computation of %s''''',char(fun{f,1}));
        end
        clear obj
